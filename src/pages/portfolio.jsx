...
function Item({ index, position, scale, c = new THREE.Color(), ...props }) {
  ...
  useFrame((state, delta) => {
    const y = scroll.curve(index / urls.length - 1.5 / urls.length, 4 / urls.length)
    ref.current.material.scale[1] = ref.current.scale.y = damp(ref.current.scale.y, clicked === index ? 6 : 4 + y, 3, delta)
...
export const App = () => (
  <>
  <Canvas style={{width:"100vw",height:"100vh"}}gl={{ antialias: false }} dpr={[1, 1.5]} onPointerMissed={() => (state.clicked = null)}>
    <Items />
  </Canvas>
  </>
)
